            ...
            {/* {account == null || (props.data.account.ownerAddress.toString() != account.toString()) ? */}
                <div className="col-3 mt-3" >
                    <Card className=''>
                        ...
                        } variant="top" src={props.data.account.nftImages[0]} />
                        <Card.Body>
                            <Card.Title> <ScopeReference hexString={props.data.publicKey.toString()} type='address' /></Card.Title>
                            <Card.Text>
                                <AuctionStatus className='' type={true} />
                                <div className='mb-1'>Starting Price: {props.data.account.startingPrice} SOL</div>
                            </Card.Text>
                            <Button onClick={() => navigate(`/detail/${props.data.publicKey.toString()}`)} variant="outline-success" className="float-end">View</Button>
                        </Card.Body>
                    </Card >
                </div >
                 {/* : <></> 
